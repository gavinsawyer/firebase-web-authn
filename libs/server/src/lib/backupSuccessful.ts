 ...
 *
 * @returns
 *  A {@link boolean} for whether the user's passkey is backed up successfully or null if no passkey was found.
 */
export const backupSuccessful: (uid: string, app?: App) => Promise<boolean | null> = (uid: string, app?: App): Promise<boolean | null> => ((firestore: Firestore): Promise<boolean | null> => (firestore.collection("users").doc(uid) as DocumentReference<WebAuthnUserDocument>).get().then<boolean | null>(
