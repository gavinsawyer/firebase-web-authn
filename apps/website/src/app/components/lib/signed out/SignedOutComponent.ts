...
export class SignedOutComponent {
  ...
  public readonly signInWithPasskey: () => Promise<void>                      = (): Promise<void> => {
    this.signInWorking$.set(true);
    this.signUpFormGroup.disable();

    return this.authenticationService.signInWithPasskey().catch<never>(
  ...
  public readonly signUpSubmit: () => Promise<void>                           = async (): Promise<void> => this.signUpFormGroup.value.name ? (async (
    name: string | undefined,
    user: User | null,
  ): Promise<void> => name && user ? ((): Promise<void> => {
    this.signUpWorking$.set(true);
    this.signUpFormGroup.disable();
